...
def natural_explain(plan):
    ...
    if plan["Node Type"] == "SetOp":
        return setop_natural_explain(plan)
    if plan["Node Type"] == "Subquery Scan":
    ...
    if plan["Node Type"] == "Bitmap Index Scan":
        return bitmap_index_scan_natural_explain(plan)
    if plan["Node Type"] == "Memoize":
        return memoize_natural_explain(plan)
    if plan["Node Type"] == "Gather Merge":
